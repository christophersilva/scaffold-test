            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-6 py-4">
                ...
                <form method="POST" action="{{ route('update-video', $video->id) }}">
                    ...
                    <div>
                        ...
                        <x-jet-input id="name" class="block mt-1 w-full" type="text" name="name" required autofocus autocomplete="name" value="{{ $video->name }}"/>
                    </div>

                    <div class="mt-4">
                        <x-jet-label for="link" value="{{ __('general.link') }}" />
                        <x-jet-input id="link" class="block mt-1 w-full" type="text" name="link" required value="{{ $video->link }}" />
                    </div>

                    <div class="mt-4">
                        <x-jet-label for="summary" value="{{ __('general.summary') }}" />
                        <x-jet-input id="summary" class="block mt-1 w-full" type="text" name="summary" required value="{{ $video->summary }}" />
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <select class="form-select rounded-md shadow-sm block mt-1 w-full" id="discipline_id" name="discipline_id" required>
                            @foreach ($disciplines as $discipline)
                                <option value="{{ $discipline->id }}" {{ $video->discipline->id != $discipline->id ?: "selected"}}>{{ $discipline->name }}</option>
                            @endforeach
                        </select>
                    </div>
